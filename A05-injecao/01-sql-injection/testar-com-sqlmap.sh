...
php -S "127.0.0.1:${PORTA}" > /tmp/servidor-sqlmap-01.log 2>&1 &
PID_SERVIDOR=$!
sleep 1

SQLMAP_OPTS="--batch --level=3 --risk=2 --technique=BE --dbms=SQLite --flush-session"

# O exit code do sqlmap NAO indica se achou injecao (e sempre 0 em execucao
# normal) - por isso checamos o texto da saida.
...
echo "================================================================"
sqlmap -u "http://127.0.0.1:${PORTA}/endpoint-vulneravel.php?usuario=admin&senha=x" \
...
echo "================================================================"
sqlmap -u "http://127.0.0.1:${PORTA}/endpoint-corrigido.php?usuario=admin&senha=x" \
...
echo "  sqlmap -u \"http://127.0.0.1:${PORTA}/endpoint-vulneravel.php?usuario=admin&senha=x\" \\"
